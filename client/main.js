import { initBossIntegration, makeBossHandlers } from './entities/boss.js'
...
import { extractStraightRunPositions as _extractStraightRunPositions, findAllGroups as _findAllGroups, floodFillGroup as _floodFillGroup, hasStraightTriple as _hasStraightTriple, predictMergeResult as _predictMergeResult } from './game/engine.js'
import * as boardModule from './game/board.js'
import { ALL_TEMPLATES, COMMON_TEMPLATES, MODULE_TEMPLATES, RARE_TEMPLATES } from './game/templates.js'
// Game rules (centralized): rarity ranks, base rates and helper predicates
import { BASE_RATES, canBeFodder, isAncestralStarUpgrade, MERGE_SEQUENCE, RARITY_RANK, requiredTotalForBaseCell, sameTemplate, upgradeAncestralStars } from './game/rules.js'
import { makeSpawner } from './game/spawn.js'
import { ASSETS, BACKGROUNDS, BOSS_IMG, importedMergeChart as IMPORTED_MERGE_CHART, MINE_ASSET, TYPE_TOKENS } from './assets/index.js'
import { createModuleInstance, normalizeId, stableHash } from './entities/moduleFactory.js'
import { canBeShattered, createMineInstance, explodeMine, getMineSpawnPercent, maybeConvertToMine, isDisabledByMine as minesIsDisabledByMine, pendingExplosions, processMineCountdowns, setMineSpawnPercent } from './entities/mines.js'
...
import { renderBoard as _renderBoard, buildMiniShape, buildShape } from './ui/renderBoard.js'
...
import { getModuleDamageValue, lazyPreloadAssets, pickCommonVariant } from './utils/assets.js'
import { getGameTimerSeconds, resetGameTimer, setGameTimerStart, startGameTimer, stopGameTimer } from './utils/timer.js'
import { ensureGameTimerPlaced, initScoreUI, updateScoreUI } from './utils/ui.js'
import { adjustRightColumnToBoard } from './utils/layout.js'
import { findTemplate } from './utils/templates.js'
import { appendVoiceChannelName, ensureHintStyles, getBoardStateHash } from './utils/helpers.js'


// Import hint system
import { getCurrentHintIndex, getCurrentHintMove, getHintMove, getHintMovesList, getLastBoardState, INACTIVITY_TIMEOUT_MS, onInactivityTimeout, resetHintCyclingState, resetInactivity, setCurrentHintIndex, setHintMove, setHintSystemDependencies, setLastBoardState, setupInactivityListeners, updateHintMovesList } from './utils/hintSystem.js'


// Import selection utilities
import { isBoardSel, isInvSel } from './utils/selection.js'
...
const gameTimerSeconds = 0
const gameTimerInterval = null
const gameTimerStart = null // epoch ms when timer started
...
function ensureBoardBindings() {
  try {
    boardModule.initBoard({
      ...
      pendingExplosions,
      sleepAnimated,
      // support both calling signatures: findAllGroups(pred) or findAllGroups(board, pred) or findAllGroups(board, pred, BOARD_ROWS, BOARD_COLS)
      findAllGroups: (b, pred, rows, cols) => {
        try {
          if (typeof pred === 'undefined') { pred = b; b = board; rows = BOARD_ROWS; cols = BOARD_COLS }
          else if (typeof rows === 'undefined') { rows = BOARD_ROWS; cols = BOARD_COLS }
  ...
  } catch (e) { try { console.debug && console.debug('setHintSystemDependencies failed', e) } catch (ee) {} }
}
...
const placingFromInventory = null // type currently placing (string) or null
const commonSwapRemaining = 20 // limit of swaps that involve Commons - DEPRECATED: free swap is now permanent
...
// merge candidates functions (moved to utils/mergeCandidates.js). Initialize via factory so they use main.js runtime state.
let mergeCandidates
{
...
function initUI() {
  ...
  if (!document.getElementById('game-board')) {
    ...
    btnHint.addEventListener('click', () => {
      ...
      try {
        ...
        try { appendToDebug && appendToDebug('Hint cycling: showing move ' + (getCurrentHintIndex() + 1) + ' of ' + getHintMovesList().length + ' prioritized moves') } catch (e) {}
        renderBoard()
  ...
  try { makeBoardBindings(boardModule, setHintSystemDependencies, appendToDebug).ensureBoardBindings.bind({
    board, inventory, BOARD_ROWS, BOARD_COLS, pendingExplosions, moveOccurredThisTurn,
    awardShards, renderBoard, sleepAnimated, cascadeResolve,
    shuffleRemaining, setShuffleRemaining,
    getHintMovesList, setHintMove, getHintMove,
    findAllGroups, hasStraightTriple, extractStraightRunPositions,
    autoShatterRares, animating,
    makeDrop, maybeConvertToMine,
    bossHelper, processMineCountdowns,
    moveCount, totalShatters, setTotalShatters: v => totalShatters = v,
    checkForValidMoves, createBoard, resetHintCyclingState,
    __BOSS_MARKER: window.__BOSS_MARKER,
    __main_globals__: { hintMovesList: getHintMovesList(), currentHintIndex: getCurrentHintIndex(), lastBoardState: getLastBoardState() },
  })() } catch (e) {}
  ...
  try { setupInactivityListeners() } catch (e) { try { console.debug && console.debug('setupInactivityListeners failed', e) } catch (ee) {} }
