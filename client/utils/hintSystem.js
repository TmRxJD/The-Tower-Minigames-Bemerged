...
function updateHintMovesList() {
  ...
  if (lastBoardState !== currentBoardHash) {
    ...
    try { appendToDebug && appendToDebug('Updated hint moves list: ' + hintMovesList.length + ' moves available (common clears: ' + commonClearMoves.length + ', total valid: ' + moves.length + ')') } catch (e) {}
  }
}
...
function onInactivityTimeout() {
  ...
  if (hintMovesList.length > 0) {
    ...
    try { appendToDebug && appendToDebug('onInactivityTimeout chose move: ' + JSON.stringify(getHintMove())) } catch (e) {}
  } else {
