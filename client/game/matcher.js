...
export function hasStraightTriple(group) {
  ...
  for (const rKey in byRow) {
    const cols = Array.from(byRow[rKey])
      .map(Number)
      .sort((a, b) => a - b);
    let run = 1;
  ...
  for (const cKey in byCol) {
    const rows = Array.from(byCol[cKey])
      .map(Number)
      .sort((a, b) => a - b);
    let run = 1;
  ...
  for (const rKey in byRow) {
    ...
    for (let i = 1; i <= cols.length; i++) {
      ...
      if (cur === prev + 1) {
        ...
        if (run.length >= 3) {
          for (const c of run) {
            outMap.set(rKey + ',' + c, { r: Number(rKey),
              c });
          }
        }
...
export function floodFillGroup(board, sr, sc, includePredicate = null, BOARD_ROWS, BOARD_COLS) {
  ...
  while (stack.length) {
    ...
    if (includePredicate && !includePredicate(cell)) {
      continue;
    }
    if (groupingIsType) {
      if (!(cell.rarity === 'Common' && cell.type === startCell.type)) {
        continue;
      }
    } else {
      if (cell.templateId !== startCell.templateId) {
        continue;
      }
    }
    out.push({ r,
  ...
  for (let r = 0; r < BOARD_ROWS; r++) {
    for (let c = 0; c < BOARD_COLS; c++) {
      ...
      while (stack.length) {
        ...
        if (groupingIsType) {
          if (!(curCell.rarity === 'Common' && curCell.type === cell.type)) {
            continue;
          }
        } else {
          if (curCell.templateId !== cell.templateId) {
            continue;
          }
        }
        seen.add(k);
