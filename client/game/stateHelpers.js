...
export function makeStateHelpers(ctx) {
  ...
  const storage = makeStorageHelpers({ get,
    set,
    localStorageRef,
    bossHelper,
    ensureBoardBindings });
  ...
  function awardShards(shType, shards) {
    try {
      ...
      try {
        set(
          'score',
          Object.values(cur || {}).reduce((a, b) => a + (Number(b) || 0), 0),
        );
      } catch (e) {}
      try {
        updateScoreUI();
      ...
      try {
        appendToDebug && appendToDebug(`awardShards: ${shType} += ${n} (total=${cur[shType]})`);
      } catch (e) {}
      try {
        if (typeof window !== 'undefined') {
          window.lastAwardedDebug = { type: shType,
            added: n,
            total: cur[shType] };
        }
      } catch (e) {}
    } catch (e) {
      console.error('awardShards failed', e);
    }
  }
