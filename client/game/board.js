...
export function get(k) {
  ...
  try {
    if (typeof v === 'function' && v.length === 0) return v()
  } catch (e) {}
  return v
}
...
export function collapseColumns() { return cascade.collapseColumns(get) }

export function refillBoard(makeDrop) { return cascade.refillBoard(get, makeDrop) }
...
export async function resolveImmediateShatters() { return cascade.resolveImmediateShatters(get) }
    ...
    for (let c = 0; c < BOARD_COLS; c++) {
      ...
      for (const dir of directions) {
        ...
        if (nr >= 0 && nr < BOARD_ROWS && nc >= 0 && nc < BOARD_COLS) {
          ...
          const groups = get('findAllGroups')(tempBoard, cell => {
            if (!cell) return false
            if (cell.rarity === 'Common') return true
            if (cell.rarity === 'Rare' && !cell.plus) return !!get('autoShatterRares')
            return false
