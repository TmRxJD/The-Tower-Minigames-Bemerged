...
export function makeSpawner(ctx) {
  ...
  const BASE_RATES = ctx && ctx.BASE_RATES ? ctx.BASE_RATES : { Common: 0.685,
    Rare: 0.29,
    Epic: 0.025 };
  ...
  const localStorageRef =
    ctx && typeof ctx.localStorage !== 'undefined'
      ? ctx.localStorage
  ...
  function rollRarity() {
    const r = rand();
    if (r < BASE_RATES.Common) {
      return 'Common';
    }
    if (r < BASE_RATES.Common + BASE_RATES.Rare) {
      return 'Rare';
    }
    return 'Epic';
  }
  ...
  function spawnOfRarity(rarity) {
    ...
    if (candidates.length === 0) {
      ...
      if (fallback.length === 0) {
        throw new Error('No candidate templates for rarity ' + rarity);
      }
