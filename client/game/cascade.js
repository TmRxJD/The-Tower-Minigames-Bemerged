...
import { extractStraightRunPositions as _extractStraightRunPositions, findAllGroups as _findAllGroups, hasStraightTriple as _hasStraightTriple } from './matcher.js'
...
export async function cascadeResolve(get) {
  ...
  const makeDrop = get('makeDrop'); const maybeConvertToMine = get('maybeConvertToMine')
    ...
    while (true) {
      ...
      for (let c = 0; c < BOARD_COLS; c++) {
        for (let r = 0; r < BOARD_ROWS; r++) {
          if (!board[r][c]) {
            board[r][c] = makeDrop(); if (board[r][c]) board[r][c].spawning = true; filled = true
          }
        }
      }
      ...
      for (let r = 0; r < BOARD_ROWS; r++) {
        ...
        for (let c = 0; c < BOARD_COLS; c++) {
          ...
          if (cell) {
            row.push({ rarity: cell.rarity, type: cell.type, templateId: cell.templateId })
          } else {
            row.push(null)
          }
        }
      ...
      const groups = _findAllGroups(board, cell => { if (!cell) return false; if (cell.templateId === '__MINE__' || cell.rarity === 'Mine') return false; if (cell.rarity === 'Common') return true; if (cell.rarity === 'Rare' && !cell.plus) return !!autoShatterRares; return false }, BOARD_ROWS, BOARD_COLS)
      ...
      for (let i = 0; i < groups.length; i++) {
        ...
        console.log(`Group ${i} cells:`, cells.map(c => ({ rarity: c.rarity, type: c.type, templateId: c.templateId })))
      }
      ...
      for (const pc of posCells) { const rem = pc.cell; if (rem) { try { const shardMap = { 'Common': 5, 'Rare': 10, 'Epic': 40 }; awardShards && awardShards(rem.type || 'Unknown', shardMap[rem.rarity] || 0) } catch (e) {} board[pc.r][pc.c] = null } }
    ...
    try {
      ...
      if (pendingExplosions && pendingExplosions.length > 0) {
        ...
        for (const ex of queue) {
          ...
          renderBoard && renderBoard(); await (sleepAnimated && sleepAnimated(180))
        ...
        try { cascadeResolve(get) } catch (e) {}
        return
      }
...
export async function resolveImmediateShatters(get) {
  ...
  const groups = _findAllGroups(board, cell => {
    if (!cell) return false
    if (cell.rarity === 'Common') return true
    if (cell.rarity === 'Rare' && !cell.plus) return !!autoShatterRares
    return false
  }, BOARD_ROWS, BOARD_COLS).filter(g => g.length >= 3 && _hasStraightTriple(g))
      ...
      if (rem) {
        ...
        if (!(rem.templateId === '__MINE__' || rem.rarity === 'Mine')) {
          try {
            ...
            if (setTotalShatters) setTotalShatters(newTotal)
          } catch (e) {}
        }
      }
      board[p.r][p.c] = null
