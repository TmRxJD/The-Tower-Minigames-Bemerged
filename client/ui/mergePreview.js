...
export function makeMergePreview(ctx) {
  const { getBoard, getSelected, getInventory, buildMiniShape, predictMergeResult, requiredTotalForBaseCell, appendToDebug, evaluateMergeAttempt, isDisabledByMine, getMergeSelecting, clearCandidateHighlights, getMineShatterSelecting, getBossShatterSelecting, getCandidateHighlights, performMineShatter, damageBossWithModules, findBossShatterCandidates, renderBoard, renderInventory, cascadeResolve, bossHelper, useVectorOnly, RARITY_COLOR, ASSETS, HARDCODED_COMMON_SELECTION, MINE_ASSET } = ctx
  ...
  function updateMergePreview(alwaysShow = false) {
    ...
    if (getMineShatterSelecting && getMineShatterSelecting()) {
      ...
      for (let i = 0; i < slots.length; i++) {
        ...
        if (info.cell && info.cell.rarity && info.cell.rarity !== 'Common') {
          ...
          slot.appendChild(buildMiniShape(info.cell, miniSize, { useVectorOnly, RARITY_COLOR, ASSETS, HARDCODED_COMMON_SELECTION, MINE_ASSET, appendToDebug }))
    ...
    if (getBossShatterSelecting && getBossShatterSelecting()) {
      ...
      if (bossR !== undefined && bossC !== undefined) {
        ...
        if (selectedModules.length > 0) {
          for (const sel of selectedModules) {
            if (sel.r >= 0 && sel.r < 8 && sel.c >= 0 && sel.c < 14) { // BOARD_ROWS, BOARD_COLS
      ...
      if (canDamage || selectedModules.length > 0) {
        const slots = canDamage ? [{ kind: 'damage', cell: null }] : [{ kind: 'invalid', cell: null }]
          ...
          if (info.kind === 'damage') {
            ...
            slot.addEventListener('click', ev => {
              ...
              if (bossCell) {
                ...
                if (cascadeResolve) cascadeResolve()
              }
            })
      ...
      try { if (bossHelper && bossHelper.renderOverlay) bossHelper.renderOverlay() } catch (e) {}
