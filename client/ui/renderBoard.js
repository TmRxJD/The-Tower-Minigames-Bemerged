...
export function renderBoard(dependencies = {}) {
  ...
  while (boardDiv.children.length < total) {
    ...
    el.addEventListener('pointerdown', function(ev) {
      ...
      self._longPressTimer = setTimeout(() => { 
        self._longPressTimer = null; 
        try { self._longPressed = true; } catch(e){} 
    ...
    el.addEventListener('pointermove', function(ev) { 
      if (!this._pointerInfo.pointerDown) return; 
  ...
  for (let r = 0; r < BOARD_ROWS; r++) {
    for (let c = 0; c < BOARD_COLS; c++) {
      ...
      if (prevId === curId) {
        ...
        try {
          ...
          if (bm && bm.r === r && bm.c === c) {
            ...
            if (existingDamagePreview) existingDamagePreview.remove();
      ...
      if (cell) {
        const SHAPE_SIZE = 60;
        const shape = buildShape(cell, SHAPE_SIZE, { useVectorOnly, RARITY_COLOR, ASSETS, HARDCODED_COMMON_SELECTION, MINE_ASSET, appendToDebug });
...
export function buildMiniShape(cell, size = 36, { useVectorOnly, RARITY_COLOR, ASSETS, HARDCODED_COMMON_SELECTION, MINE_ASSET, appendToDebug } = {}) {
  ...
  if (frameSrc) {
    ...
    imgFrame.addEventListener('load', () => {
      usedImage = true;
      try { const v = wrapper.querySelector('.vector-mini'); if (v) v.remove(); } catch (e) {}
    });
...
export function buildShape(cell, size = 56, { useVectorOnly, RARITY_COLOR, ASSETS, HARDCODED_COMMON_SELECTION, MINE_ASSET, appendToDebug } = {}) {
      ...
      if (!(moduleFoundKey && moduleFoundKey.startsWith('common_'))) {
        ...
        // 2) try template-specific common in this dir
          ...
          if (localCommons.length > 0) { localCommons.sort(); moduleFoundKey = localCommons[0]; moduleSrc = map[moduleFoundKey]; }
            ...
            if (found) { moduleFoundKey = found.key; moduleSrc = ASSETS[found.dir][found.key]; }
            else { if (ASSETS[dirKey] && ASSETS[dirKey]['mf_empty']) { frameSrc = ASSETS[dirKey]['mf_empty']; } else { frameSrc = null; } }
          }
        }
      }
  ...
  function renderVector() {
    ...
    if (cell.shape === 'diamond') {
      ...
      if (cell.shape === 'square') s.style.borderRadius = '4px';
      if (cell.shape === 'triangle') s.style.clipPath = 'polygon(50% 6%, 94% 94%, 6% 94%)';
      if (cell.plus) s.style.boxShadow = '0 0 12px rgba(255,255,255,0.9)';
      v.appendChild(s);
    }
    wrapper.appendChild(v);
  }
