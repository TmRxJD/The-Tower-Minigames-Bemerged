export function makeScoreUI(ctx) {
    ...
    try {
      ...
      try {
        ...
        if (savedStart && !isNaN(parseInt(savedStart, 10))) {
          try { window.gameTimerStart = parseInt(savedStart, 10); } catch (e) {}
          try { window.gameTimerSeconds = Math.floor((Date.now() - window.gameTimerStart) / 1000); } catch (e) {}
          timer.textContent = formatTimer(window.gameTimerSeconds);
        } else {
          timer.textContent = formatTimer(window.gameTimerSeconds || 0);
        }
      } catch (e) { try { timer.textContent = formatTimer(window.gameTimerSeconds || 0); } catch (ee) {} }
      const running = localStorage.getItem('gameTimerRunning');
      if (running === '1' && !window.gameTimerInterval) startGameTimer();
    } catch (e) {}
  }
  ...
  function updateScoreUI() {
    try {
      ...
      const movesValue = document.createElement('span'); movesValue.className = 'score-value'; movesValue.id = 'stat-move-count'; movesValue.textContent = String(state.moveCount || 0);
      ...
      movesLine.appendChild(movesLabel); movesLine.appendChild(movesValue); el.appendChild(movesLine);
      ...
      const mergesValue = document.createElement('span'); mergesValue.className = 'score-value'; mergesValue.id = 'stat-total-merges'; mergesValue.textContent = String(state.totalMerges || 0);
      mergesLine.appendChild(mergesLabel); mergesLine.appendChild(mergesValue); el.appendChild(mergesLine);
      ...
      const shattersValue = document.createElement('span'); shattersValue.className = 'score-value'; shattersValue.id = 'stat-total-shatters'; shattersValue.textContent = String(state.totalShatters || 0);
      shattersLine.appendChild(shattersLabel); shattersLine.appendChild(shattersValue); el.appendChild(shattersLine);
      ...
      scoreLine.appendChild(scoreLabel); scoreLine.appendChild(scoreValue); el.appendChild(scoreLine);
      ...
      try {
        ...
        TYPE_ORDER.forEach((tName) => {
          ...
          const img = document.createElement('img'); img.src = imgToken; img.alt = tName; img.width = 36; img.height = 36; img.style.borderRadius = '4px';
          const num = document.createElement('span'); num.id = 'shard-' + tName.toLowerCase(); num.textContent = String((state.shardsEarned && state.shardsEarned[tName]) || 0); num.style.fontWeight = '700'; num.style.marginLeft = '6px';
          item.appendChild(img); item.appendChild(num);
          shardsValue.appendChild(item);
        });
        shardsLine.appendChild(shardsLabel); shardsLine.appendChild(shardsValue); el.appendChild(shardsLine);
      } catch (e) {}
    } catch (e) {}
  }

  return { updateScoreUI, ensureGameTimerPlaced };
}
