...
function makeControls(ctx) {
  ...
  function ensureControls() {
    ...
    if (!controls) {
      ...
      btnOrient.addEventListener('click', () => {
        ...
        if (hintMove) {
          try {
            ...
            const hnc = hr;
            const updatedHintMove = { ...hintMove,
              r: hnr,
              c: hnc };
            if (typeof hintMove.nr === 'number' && typeof hintMove.nc === 'number') {
              const nr2 = oldRows - 1 - hintMove.nc;
              const nc2 = hintMove.nr;
              updatedHintMove.nr = nr2;
              updatedHintMove.nc = nc2;
            }
            // Update hintMove through the setter function
            if (typeof setHintMove === 'function') {
              setHintMove(updatedHintMove);
            }
          } catch (e) {} // eslint-disable-line no-empty
        }
      ...
      try {
        animCb.checked =
          !!localStorage.getItem('disableAnimations') && localStorage.getItem('disableAnimations') !== '0';
      } catch (e) {
