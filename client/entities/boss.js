...
function ensureOverlay() {
  if (!document.getElementById('boss-overlay')) {
    const div = createEl('div', { id: 'boss-overlay' }, {
      position: 'absolute', left: '50%', transform: 'translateX(-50%)', top: '6px', zIndex: '120',
      pointerEvents: 'none', color: '#fff', fontWeight: '700', textAlign: 'center'
    });
...
function renderOverlay() {
  ...
  wrap.innerHTML = '';
  if (!boss) return;
  const barBg = createEl('div', {}, { width: '300px', height: '18px', background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '10px', overflow: 'hidden' });
  const bar = createEl('div');
  const pct = Math.max(0, Math.min(1, boss.hitsRemaining / (opts.requiredHits || 1)));
  setStyle(bar, { width: Math.round(pct * 100) + '%', height: '100%', background: 'linear-gradient(90deg,#ff6b6b,#ffcf6b)' });
  ...
  if (boss.destructiveActive) {
    const phaseText = boss.currentPhase === 1 ? 'Phase 1' : boss.currentPhase === 2 ? 'Phase 2' : boss.currentPhase === 3 ? 'Phase 3' : 'Phase 4';
    const phaseInfo = createEl('div', { 
  ...
  try {
    if (game && game.bossShatterSelecting && game.selected && game.selected.length > 0) {
      ...
      if (selectedModules.length > 0) {
        ...
        if (totalDamage > 0) {
          const damagePreview = createEl('div', { 
  ...
  } catch (e) { /* ignore errors in damage preview */ }
...
function setState(state) {
  try {
    if (!state) {
      ...
      boss = null;
      clearOverlay();
      try { game && game.setBossOnBoard && game.setBossOnBoard(null); } catch (e) {}
...
function onMoveAdvance() {
  ...
  try {
    ...
    try { game && game.appendToDebug && game.appendToDebug('onMoveAdvance tick=' + tick + ' threshold=' + threshold + ' destructiveActive=' + !!boss.destructiveActive + ' lastDestructiveTick=' + (boss.lastDestructiveTick||0) + ' phase=' + boss.currentPhase + ' phaseMoves=' + boss.phaseMoveCount); } catch (e) {}
    ...
    if (!boss.destructiveActive && ticksSinceThreshold >= 0) {
      ...
      try { game && game.appendToDebug && game.appendToDebug('Boss destructive behavior activated at move ' + tick + ' - Phase 1 started'); } catch (e) {}
    }
    ...
    if (boss.destructiveActive) {
      ...
      if (boss.currentPhase === 1) {
        ...
        if (boss.phaseMoveCount >= (opts.phaseDuration || 10)) {
          boss.currentPhase = 2;
          boss.phaseMoveCount = 0;
          try { game && game.appendToDebug && game.appendToDebug('Boss entered Phase 2 at move ' + tick); } catch (e) {}
        }
        ...
        if (boss.phaseMoveCount >= (opts.phaseDuration || 10)) {
          boss.currentPhase = 3;
          boss.phaseMoveCount = 0;
          try { game && game.appendToDebug && game.appendToDebug('Boss entered Phase 3 at move ' + tick); } catch (e) {}
        }
        ...
        if (boss.phaseMoveCount >= (opts.phaseDuration || 10)) {
          boss.currentPhase = 4;
          boss.phaseMoveCount = 0;
          try { game && game.appendToDebug && game.appendToDebug('Boss entered Phase 4 at move ' + tick); } catch (e) {}
        }
      ...
      try { game && game.appendToDebug && game.appendToDebug('onMoveAdvance shouldDestroy=' + shouldDestroy + ' phase=' + boss.currentPhase + ' frequency=' + frequency + ' phaseMoves=' + boss.phaseMoveCount); } catch (e) {}
      ...
      if (shouldDestroy && boss.lastDestructiveTick !== tick) {
        ...
        for (let dr = -1; dr <= 1; dr++) {
          for (let dc = -1; dc <= 1; dc++) {
            if (dr === 0 && dc === 0) continue; // Don't destroy the boss itself
...
function setDevOptions(o) {
  ...
  try {
    ...
    if (boss && typeof opts.requiredHits === 'number' && opts.requiredHits !== prevReq) {
      ...
      try { game && game.appendToDebug && game.appendToDebug('setDevOptions: reset boss health to ' + opts.requiredHits + ' (requiredHits changed from ' + prevReq + ')'); } catch (e) {}
    }
  } catch (e) {}
  try {
    if (boss) {
      ...
      try { game && game.appendToDebug && game.appendToDebug('setDevOptions: applied to active boss: destructiveActive=' + boss.destructiveActive + ' tick=' + tick + ' threshold=' + threshold + ' phase reset to 1'); } catch (e) {}
      try { game && game.setBossOnBoard && game.setBossOnBoard(boss); } catch (e) {}
    }
  } catch (e) {}
  renderOverlay();
...
function getBossWaveCount() { return boss ? (boss.waveCount || 0) : 0; }
...
try { window.setBossPhases = setBossPhases; } catch (e) {}

const bossHelper = { init, setState, spawnIfEligible, onShatterResolved, onMoveAdvance, renderOverlay, setDevOptions, getStateForSave, applyDamage, getBoss, getBossWaveCount, setBossPhases };
...
export function makeBossHandlers(ctx = {}) {
      ...
      for (const [dr, dc] of dirs) {
        ...
        if (n1.rarity === 'Common' && n2.rarity === 'Common' && n1.type === n2.type) {
          ...
          for (const p of removals) {
            try { /* increment stats handled by caller if needed */ } catch (e) {}
            try { const shardMap = { 'Common': 5, 'Rare': 10, 'Epic': 40 }; awardShards(p.cell.type || 'Unknown', shardMap[p.cell.rarity] || 0); } catch (e) {}
            board[p.r][p.c] = null;
          }
          try { const posCells = removals.map(p => ({ r: p.r, c: p.c, cell: p.cell })); if (bh && bh.onShatterResolved) bh.onShatterResolved(posCells); } catch (e) {}
  ...
  return { canBossBeShattered, shatterBossAtMarker };
}
...
export function initBossIntegration(gameApi, devOptions = {}) {
  try {
    ...
    try {
      ...
      if (raw) {
        ...
        if (st && st.settings) {
          ...
          if (!restoredDevOpts.destructionThreshold && st.settings.devBossThreshold) restoredDevOpts.destructionThreshold = parseInt(st.settings.devBossThreshold,10) || undefined;
        }
      }
    } catch (e) {}

    // Initialize boss helper with the game API
    bossHelper.init(gameApi, { ...restoredDevOpts, ...devOptions });
    ...
    try {
      ...
      if (rawMain) {
        ...
        if (st && st.bossState) {
          ...
          try { bossHelper.setState(bs); } catch (e) {}
          // apply saved opts if present
          if (bs.opts && typeof bs.opts === 'object') {
            try { bossHelper.setDevOptions && bossHelper.setDevOptions(bs.opts); gameApi.appendToDebug && gameApi.appendToDebug('Restored boss opts from main save'); } catch (e) {}
          }
        }
      }
    ...
    try {
      ...
      try { console.debug('initBossIntegration: loaded devBoss keys', { s, h, mt, dt, pd }); } catch (e) {}
      const opts = {};
      if (s) {
        const v = parseFloat(s);
      ...
      if (mt) opts.moveThreshold = parseInt(mt,10) || undefined;
      if (dt) opts.destructionThreshold = parseInt(dt,10) || undefined;
      if (pd) opts.phaseDuration = parseInt(pd,10) || undefined;
      if (Object.keys(opts).length > 0) {
        try { bossHelper.setDevOptions && bossHelper.setDevOptions(opts); gameApi.appendToDebug && gameApi.appendToDebug('Applied persisted boss dev opts: ' + JSON.stringify(opts)); console.debug('Applied persisted boss dev opts', opts); } catch (e) { console.error('Failed applying persisted boss dev opts', e); }
      }
