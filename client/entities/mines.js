  ...
  try {
    ...
    for (let dr = -1; dr <= 1; dr++) {
      for (let dc = -1; dc <= 1; dc++) {
        ...
        if (nr < 0 || nc < 0 || nr >= BOARD_ROWS || nc >= BOARD_COLS) {
          continue;
        }
        const nb = board[nr] && board[nc] ? board[nr][nc] : null;
        if (nb && nb.templateId === '__MINE__') {
          return true;
        }
      }
    }
...
export function canBeShattered(cell, board, BOARD_ROWS, BOARD_COLS) {
  try {
    ...
    if (cell.templateId === '__MINE__' || cell.rarity === 'Mine') {
      if (!board || cell.r === null || cell.c === null) {
        return false;
      }
...
try {
  const saved = typeof localStorage !== 'undefined' ? localStorage.getItem('devMineRate') : null;
  if (saved) {
...
export function setMineSpawnPercent(percent) {
  ...
  try {
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem('devMineRate', MINE_SPAWN_PERCENT.toString());
    }
  } catch (e) {}
}
  ...
  setTimeout(() => {
      ...
      try {
        ...
        if (rem) {
          ...
          if (!(rem.templateId === '__MINE__' || rem.rarity === 'Mine')) {
            try {
              if (typeof totalShatters === 'function') {
                totalShatters(prev => (typeof prev === 'number' ? prev + 1 : 1));
              }
            } catch (e) {}
            try {
    ...
    if (bossDamage > 0) {
      try {
        ...
        import('./boss.js')
          .then(({ default: bossHelper }) => {
            if (bossHelper && bossHelper.applyDamage) {
              bossHelper.applyDamage(bossDamage);
              if (appendToDebug) {
                appendToDebug('Boss damaged by mine explosion for ' + bossDamage + ' hits');
              }
            }
          })
